  ...
  class liquidGLRenderer {
    constructor(snapshotSelector, snapshotResolution = 1.0) {
      ...
      this._workerEnabled =
        ...
        window.location.protocol !== "file:"; // Disable on file:// to prevent security errors
    ...
    async captureSnapshot() {
      ...
      if (typeof html2canvas === "undefined") {
        console.warn("liquidGL: html2canvas is not defined. Please ensure html2canvas is loaded before liquidGL.");
        return;
      }
      ...
      const attemptCapture = async (
        ...
        try {
          ...
          if (e.name === "SecurityError") {
            console.error(
              "liquidGL: SecurityError detected during snapshot capture. This is usually caused by running via 'file://' protocol or loading Cross-Origin images without proper headers."
            );
            if (window.location.protocol === "file:") {
              console.warn(
                "liquidGL: Please run your project on a local server (localhost). WebGL cannot load images from the file system due to browser security restrictions."
              );
              alert(
                "liquidGL Error: The effect cannot run directly from the file system due to browser security restrictions on WebGL textures. Please view this project using a local server (localhost)."
              );
            }
            return false;
          }
